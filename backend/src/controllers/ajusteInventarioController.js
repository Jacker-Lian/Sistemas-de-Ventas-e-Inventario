...
const AjusteInventarioController = {
    ...
    // Crear un nuevo ajuste de inventario (estricto y robusto)
    crearAjuste: async (req, res) => {
        // Obtener el id_usuario autenticado (preferente)
        ...
        // Validación de que la cantidad sea un número válido y diferente de cero
        const cantidadNumerica = parseInt(cantidad_ajustada);
        if (isNaN(cantidadNumerica) || cantidadNumerica <= 0) {
            return res.status(400).json({
                success: false,
                message: 'La cantidad debe ser un número entero positivo y diferente de cero.'
            });
        }

        // Validación de lógica: tipo de ajuste vs signo de cantidad (ninguna cantidad negativa permitida)
        if ((tipo_ajuste === 'AUMENTO' && cantidadNumerica <= 0) || 
            (tipo_ajuste === 'DISMINUCION' && cantidadNumerica <= 0)) {
            return res.status(400).json({
                success: false,
                message: 'El tipo de ajuste no coincide con el valor de la cantidad (debe ser positivo y acorde).'
            });
        }

        try {
            // El modelo solamente espera la cantidad en positivo
    ...
    // Obtener todos los ajustes de inventario con filtros y paginación
    obtenerTodosLosAjustes: async (req, res) => {
        try {
            // Recopila filtros de la query
            const filtros = {};
    ...
    // Obtener ajustes por producto específico, con filtros
