...
class UsuarioModel {
  ...
  async obtenerPorEmail(email) {
    try {
      ...
      const [rows] = await pool.query(
        `SELECT id_usuario, nombre_usuario, email_usuario, password_hash, rol_usuario, estado FROM ${this.table} WHERE email_usuario = ? LIMIT 1`,
        [email]
      );
      return rows[0];
    } catch (error) {
  ...
  async desactivarUsuario(id) {
    try {
      const pool = database.getPool();
      const [result] = await pool.query(`UPDATE ${this.table} SET estado = 0 WHERE id_usuario = ?`, [id]);
      return result.affectedRows > 0;
    } catch (error) {
