...
class ProductoModel {
  ...
  async obtenerProductoPorId(id) {
    const query = `
      SELECT id_producto AS id, nombre, precio_venta, precio_compra, stock, descripcion, estado, id_categoria, id_proveedor
      FROM producto
  ...
  async crearProducto(producto) {
    const query = `
      INSERT INTO producto (nombre, precio_venta, precio_compra, stock, descripcion, id_categoria, id_proveedor, estado)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  async actualizarProducto(id, producto) {
    const query = `
      UPDATE producto
      SET nombre = ?, precio_venta = ?, precio_compra = ?, stock = ?, descripcion = ?, id_categoria = ?, id_proveedor = ?
      WHERE id_producto = ?
    `;
    const [result] = await this.pool.query(query, [
      producto.nombre,
  ...
  // Método para actualizar stock (usado por ventas)
